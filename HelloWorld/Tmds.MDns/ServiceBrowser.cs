...
namespace Tmds.MDns
{
    ...
    {
        public ServiceBrowser(int newPort, String newDomain)
        {
			port = newPort;
			domain = newDomain;
            ...
            lock (interfaceHandlers)
            {
                ...
                foreach (NetworkInterface networkInterface in networkInterfaces)
                {
                    ...
                    if (interfaceHandler == null)
                    {
						...
						interfaceHandler.StartBrowse(_serviceTypes.Select(st => new Name(st.ToLower() + "." + domain + ".")));
                    }
                    if (networkInterface.OperationalStatus == OperationalStatus.Up)
                    {
                        interfaceHandler.Enable(port);
                    }
                    else
                    {
                        interfaceHandler.Disable();
